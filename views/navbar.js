define([
  ...
  var navbarView = Backbone.View.extend({
    ...
    initialize: function(){
      ...
      this.$el.html(template({navs: config.navs.models, title: config.title, icon: config.icon, wiki: config.tools.findWhere({"title":"Tutorial"}), contact: config.contact}));
    ...
    logged_in: function(){
      ...
      oh.user.whoami().done(function(username){
        ...
        oh.user.read({user_list : username}).done(function(r){
		  ...
		  user_details['permissions']['can_create_classes'] ? $("#admin-link4").show() : $("#admin-link4").hide();
